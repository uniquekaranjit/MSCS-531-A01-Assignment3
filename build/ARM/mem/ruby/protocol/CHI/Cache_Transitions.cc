 ...
 *   src/mem/slicc/symbols/StateMachine.py:1949
 */
                                        ...
                                        Addr addr)
{
  ...
  case HASH_FUN(Cache_State_BUSY_INTR, Cache_Event_DelayAtomic):
  case HASH_FUN(Cache_State_BUSY_BLKD, Cache_Event_DelayAtomic):
    Pop_TriggerQueue(m_tbe_ptr, m_cache_entry_ptr, addr);
    DelayAtomic(m_tbe_ptr, m_cache_entry_ptr, addr);
    ProcessNextState_ClearPending(m_tbe_ptr, m_cache_entry_ptr, addr);
    return TransitionResult_Valid;
  ...
  case HASH_FUN(Cache_State_I, Cache_Event_ReadNoSnp):
    next_state = Cache_State_BUSY_BLKD; m_curTransitionNextState = next_state;
    Initiate_Request(m_tbe_ptr, m_cache_entry_ptr, addr);
    Initiate_ReadNoSnp(m_tbe_ptr, m_cache_entry_ptr, addr);
    Profile_Miss(m_tbe_ptr, m_cache_entry_ptr, addr);
    Pop_ReqRdyQueue(m_tbe_ptr, m_cache_entry_ptr, addr);
    ProcessNextState(m_tbe_ptr, m_cache_entry_ptr, addr);
    return TransitionResult_Valid;

  case HASH_FUN(Cache_State_UD, Cache_Event_ReadNoSnp):
  case HASH_FUN(Cache_State_SD, Cache_Event_ReadNoSnp):
  case HASH_FUN(Cache_State_UC, Cache_Event_ReadNoSnp):
  case HASH_FUN(Cache_State_SC, Cache_Event_ReadNoSnp):
    next_state = Cache_State_BUSY_BLKD; m_curTransitionNextState = next_state;
    Initiate_Request(m_tbe_ptr, m_cache_entry_ptr, addr);
    Initiate_ReadNoSnp_Hit(m_tbe_ptr, m_cache_entry_ptr, addr);
    Profile_Hit(m_tbe_ptr, m_cache_entry_ptr, addr);
    Pop_ReqRdyQueue(m_tbe_ptr, m_cache_entry_ptr, addr);
    ProcessNextState(m_tbe_ptr, m_cache_entry_ptr, addr);
    return TransitionResult_Valid;
  ...
  case HASH_FUN(Cache_State_UD_RSC, Cache_Event_MakeReadUnique):
  case HASH_FUN(Cache_State_UC_RSC, Cache_Event_MakeReadUnique):
  case HASH_FUN(Cache_State_UD_RSD, Cache_Event_MakeReadUnique):
    next_state = Cache_State_BUSY_BLKD; m_curTransitionNextState = next_state;
    Initiate_Request(m_tbe_ptr, m_cache_entry_ptr, addr);
    Initiate_MakeReadUnique_Hit_InvUpstream(m_tbe_ptr, m_cache_entry_ptr, addr);
    Profile_Hit(m_tbe_ptr, m_cache_entry_ptr, addr);
    Pop_ReqRdyQueue(m_tbe_ptr, m_cache_entry_ptr, addr);
    ProcessNextState(m_tbe_ptr, m_cache_entry_ptr, addr);
    return TransitionResult_Valid;

  case HASH_FUN(Cache_State_RUSD, Cache_Event_MakeReadUnique):
  case HASH_FUN(Cache_State_RUSC, Cache_Event_MakeReadUnique):
  case HASH_FUN(Cache_State_RSC, Cache_Event_MakeReadUnique):
  case HASH_FUN(Cache_State_RSD, Cache_Event_MakeReadUnique):
  case HASH_FUN(Cache_State_SC_RSC, Cache_Event_MakeReadUnique):
  case HASH_FUN(Cache_State_SD_RSC, Cache_Event_MakeReadUnique):
  case HASH_FUN(Cache_State_SD_RSD, Cache_Event_MakeReadUnique):
    next_state = Cache_State_BUSY_BLKD; m_curTransitionNextState = next_state;
    Initiate_Request(m_tbe_ptr, m_cache_entry_ptr, addr);
    Initiate_MakeReadUnique_Hit_InvUpstream(m_tbe_ptr, m_cache_entry_ptr, addr);
    Profile_Miss(m_tbe_ptr, m_cache_entry_ptr, addr);
  ...
  case HASH_FUN(Cache_State_BUSY_BLKD, Cache_Event_FinishMakeReadUnique):
    Pop_TriggerQueue(m_tbe_ptr, m_cache_entry_ptr, addr);
    Finish_MakeReadUnique(m_tbe_ptr, m_cache_entry_ptr, addr);
    ProcessNextState_ClearPending(m_tbe_ptr, m_cache_entry_ptr, addr);
    return TransitionResult_Valid;

  case HASH_FUN(Cache_State_BUSY_INTR, Cache_Event_FinishMakeReadUnique):
    next_state = Cache_State_BUSY_BLKD; m_curTransitionNextState = next_state;
    Pop_TriggerQueue(m_tbe_ptr, m_cache_entry_ptr, addr);
    Finish_MakeReadUnique(m_tbe_ptr, m_cache_entry_ptr, addr);
    ProcessNextState_ClearPending(m_tbe_ptr, m_cache_entry_ptr, addr);
  ...
  case HASH_FUN(Cache_State_I, Cache_Event_WriteUniqueZero):
  case HASH_FUN(Cache_State_RU, Cache_Event_WriteUniqueZero):
  case HASH_FUN(Cache_State_RSC, Cache_Event_WriteUniqueZero):
  case HASH_FUN(Cache_State_RSD, Cache_Event_WriteUniqueZero):
  case HASH_FUN(Cache_State_RUSD, Cache_Event_WriteUniqueZero):
  case HASH_FUN(Cache_State_RUSC, Cache_Event_WriteUniqueZero):
    next_state = Cache_State_BUSY_BLKD; m_curTransitionNextState = next_state;
    Initiate_Request(m_tbe_ptr, m_cache_entry_ptr, addr);
    Initiate_WriteUnique_Zero(m_tbe_ptr, m_cache_entry_ptr, addr);
    Profile_Miss(m_tbe_ptr, m_cache_entry_ptr, addr);
    Pop_ReqRdyQueue(m_tbe_ptr, m_cache_entry_ptr, addr);
    ProcessNextState(m_tbe_ptr, m_cache_entry_ptr, addr);
    return TransitionResult_Valid;

  case HASH_FUN(Cache_State_SC, Cache_Event_WriteUniqueZero):
  case HASH_FUN(Cache_State_UC, Cache_Event_WriteUniqueZero):
  case HASH_FUN(Cache_State_SD, Cache_Event_WriteUniqueZero):
  case HASH_FUN(Cache_State_UD, Cache_Event_WriteUniqueZero):
  case HASH_FUN(Cache_State_SC_RSC, Cache_Event_WriteUniqueZero):
  case HASH_FUN(Cache_State_UC_RSC, Cache_Event_WriteUniqueZero):
  case HASH_FUN(Cache_State_SD_RSC, Cache_Event_WriteUniqueZero):
  case HASH_FUN(Cache_State_UD_RSC, Cache_Event_WriteUniqueZero):
  case HASH_FUN(Cache_State_UC_RU, Cache_Event_WriteUniqueZero):
  case HASH_FUN(Cache_State_UD_RU, Cache_Event_WriteUniqueZero):
  case HASH_FUN(Cache_State_UD_RSD, Cache_Event_WriteUniqueZero):
  case HASH_FUN(Cache_State_SD_RSD, Cache_Event_WriteUniqueZero):
    next_state = Cache_State_BUSY_BLKD; m_curTransitionNextState = next_state;
    Initiate_Request(m_tbe_ptr, m_cache_entry_ptr, addr);
    Initiate_WriteUnique_Zero(m_tbe_ptr, m_cache_entry_ptr, addr);
    Profile_Hit(m_tbe_ptr, m_cache_entry_ptr, addr);
  ...
  case HASH_FUN(Cache_State_I, Cache_Event_AtomicReturn):
  case HASH_FUN(Cache_State_SC, Cache_Event_AtomicReturn):
  case HASH_FUN(Cache_State_SC_RSC, Cache_Event_AtomicReturn):
  case HASH_FUN(Cache_State_SD, Cache_Event_AtomicReturn):
  case HASH_FUN(Cache_State_SD_RSD, Cache_Event_AtomicReturn):
  case HASH_FUN(Cache_State_SD_RSC, Cache_Event_AtomicReturn):
  case HASH_FUN(Cache_State_RSD, Cache_Event_AtomicReturn):
  case HASH_FUN(Cache_State_RUSD, Cache_Event_AtomicReturn):
  case HASH_FUN(Cache_State_RUSC, Cache_Event_AtomicReturn):
  case HASH_FUN(Cache_State_UD, Cache_Event_AtomicReturn):
  case HASH_FUN(Cache_State_UD_RSC, Cache_Event_AtomicReturn):
  case HASH_FUN(Cache_State_UD_RSD, Cache_Event_AtomicReturn):
  case HASH_FUN(Cache_State_UD_RU, Cache_Event_AtomicReturn):
  case HASH_FUN(Cache_State_UC, Cache_Event_AtomicReturn):
  case HASH_FUN(Cache_State_UC_RSC, Cache_Event_AtomicReturn):
  case HASH_FUN(Cache_State_UC_RU, Cache_Event_AtomicReturn):
  case HASH_FUN(Cache_State_RSC, Cache_Event_AtomicReturn):
  case HASH_FUN(Cache_State_RU, Cache_Event_AtomicReturn):
    next_state = Cache_State_BUSY_BLKD; m_curTransitionNextState = next_state;
    Initiate_Request(m_tbe_ptr, m_cache_entry_ptr, addr);
    Initiate_AtomicReturn_Forward(m_tbe_ptr, m_cache_entry_ptr, addr);
    Profile_Miss(m_tbe_ptr, m_cache_entry_ptr, addr);
    Pop_ReqRdyQueue(m_tbe_ptr, m_cache_entry_ptr, addr);
    ProcessNextState(m_tbe_ptr, m_cache_entry_ptr, addr);
    return TransitionResult_Valid;

  case HASH_FUN(Cache_State_I, Cache_Event_AtomicNoReturn):
  case HASH_FUN(Cache_State_SC, Cache_Event_AtomicNoReturn):
  case HASH_FUN(Cache_State_SC_RSC, Cache_Event_AtomicNoReturn):
  case HASH_FUN(Cache_State_SD, Cache_Event_AtomicNoReturn):
  case HASH_FUN(Cache_State_SD_RSD, Cache_Event_AtomicNoReturn):
  case HASH_FUN(Cache_State_SD_RSC, Cache_Event_AtomicNoReturn):
  case HASH_FUN(Cache_State_RSD, Cache_Event_AtomicNoReturn):
  case HASH_FUN(Cache_State_RUSD, Cache_Event_AtomicNoReturn):
  case HASH_FUN(Cache_State_RUSC, Cache_Event_AtomicNoReturn):
  case HASH_FUN(Cache_State_UD, Cache_Event_AtomicNoReturn):
  case HASH_FUN(Cache_State_UD_RSC, Cache_Event_AtomicNoReturn):
  case HASH_FUN(Cache_State_UD_RSD, Cache_Event_AtomicNoReturn):
  case HASH_FUN(Cache_State_UD_RU, Cache_Event_AtomicNoReturn):
  case HASH_FUN(Cache_State_UC, Cache_Event_AtomicNoReturn):
  case HASH_FUN(Cache_State_UC_RSC, Cache_Event_AtomicNoReturn):
  case HASH_FUN(Cache_State_UC_RU, Cache_Event_AtomicNoReturn):
  case HASH_FUN(Cache_State_RSC, Cache_Event_AtomicNoReturn):
  case HASH_FUN(Cache_State_RU, Cache_Event_AtomicNoReturn):
    next_state = Cache_State_BUSY_BLKD; m_curTransitionNextState = next_state;
    Initiate_Request(m_tbe_ptr, m_cache_entry_ptr, addr);
    Initiate_AtomicNoReturn_Forward(m_tbe_ptr, m_cache_entry_ptr, addr);
    Profile_Miss(m_tbe_ptr, m_cache_entry_ptr, addr);
    Pop_ReqRdyQueue(m_tbe_ptr, m_cache_entry_ptr, addr);
    ProcessNextState(m_tbe_ptr, m_cache_entry_ptr, addr);
    return TransitionResult_Valid;

  case HASH_FUN(Cache_State_UD, Cache_Event_AtomicReturn_PoC_Alloc):
  case HASH_FUN(Cache_State_UD, Cache_Event_AtomicReturn_PoC):
  case HASH_FUN(Cache_State_UD_RU, Cache_Event_AtomicReturn_PoC_Alloc):
  case HASH_FUN(Cache_State_UD_RU, Cache_Event_AtomicReturn_PoC):
  case HASH_FUN(Cache_State_UD_RSD, Cache_Event_AtomicReturn_PoC_Alloc):
  case HASH_FUN(Cache_State_UD_RSD, Cache_Event_AtomicReturn_PoC):
  case HASH_FUN(Cache_State_UD_RSC, Cache_Event_AtomicReturn_PoC_Alloc):
  case HASH_FUN(Cache_State_UD_RSC, Cache_Event_AtomicReturn_PoC):
  case HASH_FUN(Cache_State_UC, Cache_Event_AtomicReturn_PoC_Alloc):
  case HASH_FUN(Cache_State_UC, Cache_Event_AtomicReturn_PoC):
  case HASH_FUN(Cache_State_UC_RU, Cache_Event_AtomicReturn_PoC_Alloc):
  case HASH_FUN(Cache_State_UC_RU, Cache_Event_AtomicReturn_PoC):
  case HASH_FUN(Cache_State_UC_RSC, Cache_Event_AtomicReturn_PoC_Alloc):
  case HASH_FUN(Cache_State_UC_RSC, Cache_Event_AtomicReturn_PoC):
    next_state = Cache_State_BUSY_BLKD; m_curTransitionNextState = next_state;
    Initiate_Request(m_tbe_ptr, m_cache_entry_ptr, addr);
    Initiate_AtomicReturn_LocalWrite(m_tbe_ptr, m_cache_entry_ptr, addr);
    Profile_Hit(m_tbe_ptr, m_cache_entry_ptr, addr);
    Pop_ReqRdyQueue(m_tbe_ptr, m_cache_entry_ptr, addr);
    ProcessNextState(m_tbe_ptr, m_cache_entry_ptr, addr);
    return TransitionResult_Valid;

  case HASH_FUN(Cache_State_UD, Cache_Event_AtomicNoReturn_PoC_Alloc):
  case HASH_FUN(Cache_State_UD, Cache_Event_AtomicNoReturn_PoC):
  case HASH_FUN(Cache_State_UD_RU, Cache_Event_AtomicNoReturn_PoC_Alloc):
  case HASH_FUN(Cache_State_UD_RU, Cache_Event_AtomicNoReturn_PoC):
  case HASH_FUN(Cache_State_UD_RSD, Cache_Event_AtomicNoReturn_PoC_Alloc):
  case HASH_FUN(Cache_State_UD_RSD, Cache_Event_AtomicNoReturn_PoC):
  case HASH_FUN(Cache_State_UD_RSC, Cache_Event_AtomicNoReturn_PoC_Alloc):
  case HASH_FUN(Cache_State_UD_RSC, Cache_Event_AtomicNoReturn_PoC):
  case HASH_FUN(Cache_State_UC, Cache_Event_AtomicNoReturn_PoC_Alloc):
  case HASH_FUN(Cache_State_UC, Cache_Event_AtomicNoReturn_PoC):
  case HASH_FUN(Cache_State_UC_RU, Cache_Event_AtomicNoReturn_PoC_Alloc):
  case HASH_FUN(Cache_State_UC_RU, Cache_Event_AtomicNoReturn_PoC):
  case HASH_FUN(Cache_State_UC_RSC, Cache_Event_AtomicNoReturn_PoC_Alloc):
  case HASH_FUN(Cache_State_UC_RSC, Cache_Event_AtomicNoReturn_PoC):
    next_state = Cache_State_BUSY_BLKD; m_curTransitionNextState = next_state;
    Initiate_Request(m_tbe_ptr, m_cache_entry_ptr, addr);
    Initiate_AtomicNoReturn_LocalWrite(m_tbe_ptr, m_cache_entry_ptr, addr);
    Profile_Hit(m_tbe_ptr, m_cache_entry_ptr, addr);
    Pop_ReqRdyQueue(m_tbe_ptr, m_cache_entry_ptr, addr);
    ProcessNextState(m_tbe_ptr, m_cache_entry_ptr, addr);
    return TransitionResult_Valid;

  case HASH_FUN(Cache_State_SD, Cache_Event_AtomicReturn_PoC_Alloc):
  case HASH_FUN(Cache_State_SD, Cache_Event_AtomicReturn_PoC):
  case HASH_FUN(Cache_State_SD_RSD, Cache_Event_AtomicReturn_PoC_Alloc):
  case HASH_FUN(Cache_State_SD_RSD, Cache_Event_AtomicReturn_PoC):
  case HASH_FUN(Cache_State_SD_RSC, Cache_Event_AtomicReturn_PoC_Alloc):
  case HASH_FUN(Cache_State_SD_RSC, Cache_Event_AtomicReturn_PoC):
  case HASH_FUN(Cache_State_SC, Cache_Event_AtomicReturn_PoC_Alloc):
  case HASH_FUN(Cache_State_SC, Cache_Event_AtomicReturn_PoC):
  case HASH_FUN(Cache_State_SC_RSC, Cache_Event_AtomicReturn_PoC_Alloc):
  case HASH_FUN(Cache_State_SC_RSC, Cache_Event_AtomicReturn_PoC):
  case HASH_FUN(Cache_State_RSC, Cache_Event_AtomicReturn_PoC_Alloc):
  case HASH_FUN(Cache_State_RSD, Cache_Event_AtomicReturn_PoC_Alloc):
  case HASH_FUN(Cache_State_RUSC, Cache_Event_AtomicReturn_PoC_Alloc):
  case HASH_FUN(Cache_State_RUSD, Cache_Event_AtomicReturn_PoC_Alloc):
  case HASH_FUN(Cache_State_RU, Cache_Event_AtomicReturn_PoC_Alloc):
    next_state = Cache_State_BUSY_BLKD; m_curTransitionNextState = next_state;
    Initiate_Request(m_tbe_ptr, m_cache_entry_ptr, addr);
    Initiate_AtomicReturn_LocalWrite(m_tbe_ptr, m_cache_entry_ptr, addr);
    Profile_Miss(m_tbe_ptr, m_cache_entry_ptr, addr);
  ...
  case HASH_FUN(Cache_State_RSC, Cache_Event_AtomicReturn_PoC):
  case HASH_FUN(Cache_State_RSD, Cache_Event_AtomicReturn_PoC):
  case HASH_FUN(Cache_State_RUSC, Cache_Event_AtomicReturn_PoC):
  case HASH_FUN(Cache_State_RUSD, Cache_Event_AtomicReturn_PoC):
  case HASH_FUN(Cache_State_RU, Cache_Event_AtomicReturn_PoC):
    next_state = Cache_State_BUSY_BLKD; m_curTransitionNextState = next_state;
    Initiate_Request(m_tbe_ptr, m_cache_entry_ptr, addr);
    Initiate_AtomicReturn_WriteBack(m_tbe_ptr, m_cache_entry_ptr, addr);
    Profile_Miss(m_tbe_ptr, m_cache_entry_ptr, addr);
    Pop_ReqRdyQueue(m_tbe_ptr, m_cache_entry_ptr, addr);
    ProcessNextState(m_tbe_ptr, m_cache_entry_ptr, addr);
    return TransitionResult_Valid;

  case HASH_FUN(Cache_State_SD, Cache_Event_AtomicNoReturn_PoC_Alloc):
  case HASH_FUN(Cache_State_SD, Cache_Event_AtomicNoReturn_PoC):
  case HASH_FUN(Cache_State_SD_RSD, Cache_Event_AtomicNoReturn_PoC_Alloc):
  case HASH_FUN(Cache_State_SD_RSD, Cache_Event_AtomicNoReturn_PoC):
  case HASH_FUN(Cache_State_SD_RSC, Cache_Event_AtomicNoReturn_PoC_Alloc):
  case HASH_FUN(Cache_State_SD_RSC, Cache_Event_AtomicNoReturn_PoC):
  case HASH_FUN(Cache_State_SC, Cache_Event_AtomicNoReturn_PoC_Alloc):
  case HASH_FUN(Cache_State_SC, Cache_Event_AtomicNoReturn_PoC):
  case HASH_FUN(Cache_State_SC_RSC, Cache_Event_AtomicNoReturn_PoC_Alloc):
  case HASH_FUN(Cache_State_SC_RSC, Cache_Event_AtomicNoReturn_PoC):
  case HASH_FUN(Cache_State_RSC, Cache_Event_AtomicNoReturn_PoC_Alloc):
  case HASH_FUN(Cache_State_RSD, Cache_Event_AtomicNoReturn_PoC_Alloc):
  case HASH_FUN(Cache_State_RUSC, Cache_Event_AtomicNoReturn_PoC_Alloc):
  case HASH_FUN(Cache_State_RUSD, Cache_Event_AtomicNoReturn_PoC_Alloc):
  case HASH_FUN(Cache_State_RU, Cache_Event_AtomicNoReturn_PoC_Alloc):
    next_state = Cache_State_BUSY_BLKD; m_curTransitionNextState = next_state;
    Initiate_Request(m_tbe_ptr, m_cache_entry_ptr, addr);
    Initiate_AtomicNoReturn_LocalWrite(m_tbe_ptr, m_cache_entry_ptr, addr);
    Profile_Miss(m_tbe_ptr, m_cache_entry_ptr, addr);
  ...
  case HASH_FUN(Cache_State_RSC, Cache_Event_AtomicNoReturn_PoC):
  case HASH_FUN(Cache_State_RSD, Cache_Event_AtomicNoReturn_PoC):
  case HASH_FUN(Cache_State_RUSC, Cache_Event_AtomicNoReturn_PoC):
  case HASH_FUN(Cache_State_RUSD, Cache_Event_AtomicNoReturn_PoC):
  case HASH_FUN(Cache_State_RU, Cache_Event_AtomicNoReturn_PoC):
    next_state = Cache_State_BUSY_BLKD; m_curTransitionNextState = next_state;
    Initiate_Request(m_tbe_ptr, m_cache_entry_ptr, addr);
    Initiate_AtomicNoReturn_WriteBack(m_tbe_ptr, m_cache_entry_ptr, addr);
    Profile_Miss(m_tbe_ptr, m_cache_entry_ptr, addr);
    Pop_ReqRdyQueue(m_tbe_ptr, m_cache_entry_ptr, addr);
    ProcessNextState(m_tbe_ptr, m_cache_entry_ptr, addr);
    return TransitionResult_Valid;

  case HASH_FUN(Cache_State_I, Cache_Event_AtomicReturn_PoC_Alloc):
    next_state = Cache_State_BUSY_BLKD; m_curTransitionNextState = next_state;
    Initiate_Request(m_tbe_ptr, m_cache_entry_ptr, addr);
    Initiate_AtomicReturn_Miss_Alloc(m_tbe_ptr, m_cache_entry_ptr, addr);
    Allocate_DirEntry(m_tbe_ptr, m_cache_entry_ptr, addr);
    Profile_Miss(m_tbe_ptr, m_cache_entry_ptr, addr);
    Pop_ReqRdyQueue(m_tbe_ptr, m_cache_entry_ptr, addr);
    ProcessNextState(m_tbe_ptr, m_cache_entry_ptr, addr);
  ...
  case HASH_FUN(Cache_State_I, Cache_Event_AtomicNoReturn_PoC_Alloc):
    next_state = Cache_State_BUSY_BLKD; m_curTransitionNextState = next_state;
    Initiate_Request(m_tbe_ptr, m_cache_entry_ptr, addr);
    Initiate_AtomicNoReturn_Miss_Alloc(m_tbe_ptr, m_cache_entry_ptr, addr);
    Allocate_DirEntry(m_tbe_ptr, m_cache_entry_ptr, addr);
    Profile_Miss(m_tbe_ptr, m_cache_entry_ptr, addr);
    Pop_ReqRdyQueue(m_tbe_ptr, m_cache_entry_ptr, addr);
    ProcessNextState(m_tbe_ptr, m_cache_entry_ptr, addr);
  ...
  case HASH_FUN(Cache_State_I, Cache_Event_AtomicReturn_PoC):
    next_state = Cache_State_BUSY_BLKD; m_curTransitionNextState = next_state;
    Initiate_Request(m_tbe_ptr, m_cache_entry_ptr, addr);
    Initiate_AtomicReturn_Miss(m_tbe_ptr, m_cache_entry_ptr, addr);
    Allocate_DirEntry(m_tbe_ptr, m_cache_entry_ptr, addr);
    Profile_Miss(m_tbe_ptr, m_cache_entry_ptr, addr);
    Pop_ReqRdyQueue(m_tbe_ptr, m_cache_entry_ptr, addr);
    ProcessNextState(m_tbe_ptr, m_cache_entry_ptr, addr);
  ...
  case HASH_FUN(Cache_State_I, Cache_Event_AtomicNoReturn_PoC):
    next_state = Cache_State_BUSY_BLKD; m_curTransitionNextState = next_state;
    Initiate_Request(m_tbe_ptr, m_cache_entry_ptr, addr);
    Initiate_AtomicNoReturn_Miss(m_tbe_ptr, m_cache_entry_ptr, addr);
    Allocate_DirEntry(m_tbe_ptr, m_cache_entry_ptr, addr);
    Profile_Miss(m_tbe_ptr, m_cache_entry_ptr, addr);
    Pop_ReqRdyQueue(m_tbe_ptr, m_cache_entry_ptr, addr);
    ProcessNextState(m_tbe_ptr, m_cache_entry_ptr, addr);
  ...
  case HASH_FUN(Cache_State_RUSC, Cache_Event_Prefetch):
  ...
  case HASH_FUN(Cache_State_UC, Cache_Event_AtomicLoad):
  case HASH_FUN(Cache_State_UC, Cache_Event_AtomicStore):
    next_state = Cache_State_BUSY_BLKD; m_curTransitionNextState = next_state;
    Initiate_Request(m_tbe_ptr, m_cache_entry_ptr, addr);
    Initiate_Atomic_UC(m_tbe_ptr, m_cache_entry_ptr, addr);
    Profile_Hit(m_tbe_ptr, m_cache_entry_ptr, addr);
    Pop_ReqRdyQueue(m_tbe_ptr, m_cache_entry_ptr, addr);
    ProcessNextState(m_tbe_ptr, m_cache_entry_ptr, addr);
    return TransitionResult_Valid;

  case HASH_FUN(Cache_State_UD, Cache_Event_AtomicLoad):
  case HASH_FUN(Cache_State_UD, Cache_Event_AtomicStore):
  case HASH_FUN(Cache_State_UD_T, Cache_Event_AtomicLoad):
  case HASH_FUN(Cache_State_UD_T, Cache_Event_AtomicStore):
    next_state = Cache_State_BUSY_BLKD; m_curTransitionNextState = next_state;
    Initiate_Request(m_tbe_ptr, m_cache_entry_ptr, addr);
    Initiate_Atomic_UD(m_tbe_ptr, m_cache_entry_ptr, addr);
    Profile_Hit(m_tbe_ptr, m_cache_entry_ptr, addr);
    Pop_ReqRdyQueue(m_tbe_ptr, m_cache_entry_ptr, addr);
    ProcessNextState(m_tbe_ptr, m_cache_entry_ptr, addr);
    return TransitionResult_Valid;

  case HASH_FUN(Cache_State_BUSY_BLKD, Cache_Event_AtomicHit):
    Pop_TriggerQueue(m_tbe_ptr, m_cache_entry_ptr, addr);
    Callback_AtomicHit(m_tbe_ptr, m_cache_entry_ptr, addr);
    ProcessNextState_ClearPending(m_tbe_ptr, m_cache_entry_ptr, addr);
    return TransitionResult_Valid;
  ...
  case HASH_FUN(Cache_State_I, Cache_Event_AtomicLoad):
    next_state = Cache_State_BUSY_BLKD; m_curTransitionNextState = next_state;
    Initiate_Request(m_tbe_ptr, m_cache_entry_ptr, addr);
    Initiate_AtomicReturn_I(m_tbe_ptr, m_cache_entry_ptr, addr);
    Profile_Miss(m_tbe_ptr, m_cache_entry_ptr, addr);
    Pop_ReqRdyQueue(m_tbe_ptr, m_cache_entry_ptr, addr);
    ProcessNextState(m_tbe_ptr, m_cache_entry_ptr, addr);
    return TransitionResult_Valid;

  case HASH_FUN(Cache_State_I, Cache_Event_AtomicStore):
    next_state = Cache_State_BUSY_BLKD; m_curTransitionNextState = next_state;
    Initiate_Request(m_tbe_ptr, m_cache_entry_ptr, addr);
    Initiate_AtomicNoReturn_I(m_tbe_ptr, m_cache_entry_ptr, addr);
    Profile_Miss(m_tbe_ptr, m_cache_entry_ptr, addr);
    Pop_ReqRdyQueue(m_tbe_ptr, m_cache_entry_ptr, addr);
    ProcessNextState(m_tbe_ptr, m_cache_entry_ptr, addr);
    return TransitionResult_Valid;

  case HASH_FUN(Cache_State_SD, Cache_Event_AtomicLoad):
    next_state = Cache_State_BUSY_BLKD; m_curTransitionNextState = next_state;
    Initiate_Request(m_tbe_ptr, m_cache_entry_ptr, addr);
    Initiate_AtomicReturn_SD(m_tbe_ptr, m_cache_entry_ptr, addr);
    Profile_Miss(m_tbe_ptr, m_cache_entry_ptr, addr);
    Pop_ReqRdyQueue(m_tbe_ptr, m_cache_entry_ptr, addr);
    ProcessNextState(m_tbe_ptr, m_cache_entry_ptr, addr);
    return TransitionResult_Valid;

  case HASH_FUN(Cache_State_SC, Cache_Event_AtomicLoad):
    next_state = Cache_State_BUSY_BLKD; m_curTransitionNextState = next_state;
    Initiate_Request(m_tbe_ptr, m_cache_entry_ptr, addr);
    Initiate_AtomicReturn_SC(m_tbe_ptr, m_cache_entry_ptr, addr);
    Profile_Miss(m_tbe_ptr, m_cache_entry_ptr, addr);
    Pop_ReqRdyQueue(m_tbe_ptr, m_cache_entry_ptr, addr);
    ProcessNextState(m_tbe_ptr, m_cache_entry_ptr, addr);
    return TransitionResult_Valid;

  case HASH_FUN(Cache_State_SD, Cache_Event_AtomicStore):
    next_state = Cache_State_BUSY_BLKD; m_curTransitionNextState = next_state;
    Initiate_Request(m_tbe_ptr, m_cache_entry_ptr, addr);
    Initiate_AtomicNoReturn_SD(m_tbe_ptr, m_cache_entry_ptr, addr);
    Profile_Miss(m_tbe_ptr, m_cache_entry_ptr, addr);
    Pop_ReqRdyQueue(m_tbe_ptr, m_cache_entry_ptr, addr);
    ProcessNextState(m_tbe_ptr, m_cache_entry_ptr, addr);
    return TransitionResult_Valid;

  case HASH_FUN(Cache_State_SC, Cache_Event_AtomicStore):
    next_state = Cache_State_BUSY_BLKD; m_curTransitionNextState = next_state;
    Initiate_Request(m_tbe_ptr, m_cache_entry_ptr, addr);
    Initiate_AtomicNoReturn_SC(m_tbe_ptr, m_cache_entry_ptr, addr);
    Profile_Miss(m_tbe_ptr, m_cache_entry_ptr, addr);
    Pop_ReqRdyQueue(m_tbe_ptr, m_cache_entry_ptr, addr);
    ProcessNextState(m_tbe_ptr, m_cache_entry_ptr, addr);
    return TransitionResult_Valid;
  ...
  case HASH_FUN(Cache_State_SD_RSD, Cache_Event_Global_Eviction):
    next_state = Cache_State_BUSY_BLKD; m_curTransitionNextState = next_state;
    if (!checkResourceAvailable(Cache_RequestType_ReplTBEAvailable, addr)) {
        return TransitionResult_ResourceStall;
    }
  ...
  case HASH_FUN(Cache_State_I, Cache_Event_StashOnceShared):
  case HASH_FUN(Cache_State_I, Cache_Event_StashOnceUnique):
  case HASH_FUN(Cache_State_SC, Cache_Event_StashOnceShared):
  case HASH_FUN(Cache_State_SC, Cache_Event_StashOnceUnique):
  case HASH_FUN(Cache_State_UC, Cache_Event_StashOnceShared):
  case HASH_FUN(Cache_State_UC, Cache_Event_StashOnceUnique):
  case HASH_FUN(Cache_State_SD, Cache_Event_StashOnceShared):
  case HASH_FUN(Cache_State_SD, Cache_Event_StashOnceUnique):
  case HASH_FUN(Cache_State_UD, Cache_Event_StashOnceShared):
  case HASH_FUN(Cache_State_UD, Cache_Event_StashOnceUnique):
  case HASH_FUN(Cache_State_RU, Cache_Event_StashOnceShared):
  case HASH_FUN(Cache_State_RU, Cache_Event_StashOnceUnique):
  case HASH_FUN(Cache_State_RSC, Cache_Event_StashOnceShared):
  case HASH_FUN(Cache_State_RSC, Cache_Event_StashOnceUnique):
  case HASH_FUN(Cache_State_RSD, Cache_Event_StashOnceShared):
  case HASH_FUN(Cache_State_RSD, Cache_Event_StashOnceUnique):
  case HASH_FUN(Cache_State_RUSD, Cache_Event_StashOnceShared):
  case HASH_FUN(Cache_State_RUSD, Cache_Event_StashOnceUnique):
  case HASH_FUN(Cache_State_SC_RSC, Cache_Event_StashOnceShared):
  case HASH_FUN(Cache_State_SC_RSC, Cache_Event_StashOnceUnique):
  case HASH_FUN(Cache_State_UC_RSC, Cache_Event_StashOnceShared):
  case HASH_FUN(Cache_State_UC_RSC, Cache_Event_StashOnceUnique):
  case HASH_FUN(Cache_State_SD_RSC, Cache_Event_StashOnceShared):
  case HASH_FUN(Cache_State_SD_RSC, Cache_Event_StashOnceUnique):
  case HASH_FUN(Cache_State_UD_RSC, Cache_Event_StashOnceShared):
  case HASH_FUN(Cache_State_UD_RSC, Cache_Event_StashOnceUnique):
  case HASH_FUN(Cache_State_UC_RU, Cache_Event_StashOnceShared):
  case HASH_FUN(Cache_State_UC_RU, Cache_Event_StashOnceUnique):
  case HASH_FUN(Cache_State_UD_RU, Cache_Event_StashOnceShared):
  case HASH_FUN(Cache_State_UD_RU, Cache_Event_StashOnceUnique):
  case HASH_FUN(Cache_State_UD_RSD, Cache_Event_StashOnceShared):
  case HASH_FUN(Cache_State_UD_RSD, Cache_Event_StashOnceUnique):
  case HASH_FUN(Cache_State_SD_RSD, Cache_Event_StashOnceShared):
  case HASH_FUN(Cache_State_SD_RSD, Cache_Event_StashOnceUnique):
  case HASH_FUN(Cache_State_RUSC, Cache_Event_StashOnceShared):
  case HASH_FUN(Cache_State_RUSC, Cache_Event_StashOnceUnique):
    next_state = Cache_State_BUSY_BLKD; m_curTransitionNextState = next_state;
    if (!(*m_rspOut_ptr).areNSlotsAvailable(1, clockEdge()))
        return TransitionResult_ResourceStall;
    Initiate_Request(m_tbe_ptr, m_cache_entry_ptr, addr);
  ...
  case HASH_FUN(Cache_State_BUSY_BLKD, Cache_Event_ReadNoSnp):
  ...
  case HASH_FUN(Cache_State_BUSY_BLKD, Cache_Event_AtomicLoad):
  case HASH_FUN(Cache_State_BUSY_BLKD, Cache_Event_AtomicStore):
  case HASH_FUN(Cache_State_BUSY_BLKD, Cache_Event_Prefetch):
  case HASH_FUN(Cache_State_BUSY_BLKD, Cache_Event_MakeReadUnique):
  ...
  case HASH_FUN(Cache_State_BUSY_BLKD, Cache_Event_WriteUniqueZero):
  case HASH_FUN(Cache_State_BUSY_BLKD, Cache_Event_AtomicReturn):
  case HASH_FUN(Cache_State_BUSY_BLKD, Cache_Event_AtomicReturn_PoC):
  case HASH_FUN(Cache_State_BUSY_BLKD, Cache_Event_AtomicReturn_PoC_Alloc):
  case HASH_FUN(Cache_State_BUSY_BLKD, Cache_Event_AtomicNoReturn):
  case HASH_FUN(Cache_State_BUSY_BLKD, Cache_Event_AtomicNoReturn_PoC):
  case HASH_FUN(Cache_State_BUSY_BLKD, Cache_Event_AtomicNoReturn_PoC_Alloc):
  case HASH_FUN(Cache_State_BUSY_BLKD, Cache_Event_StashOnceShared):
  case HASH_FUN(Cache_State_BUSY_BLKD, Cache_Event_StashOnceUnique):
  ...
  case HASH_FUN(Cache_State_BUSY_INTR, Cache_Event_ReadNoSnp):
  ...
  case HASH_FUN(Cache_State_BUSY_INTR, Cache_Event_AtomicLoad):
  case HASH_FUN(Cache_State_BUSY_INTR, Cache_Event_AtomicStore):
  case HASH_FUN(Cache_State_BUSY_INTR, Cache_Event_Prefetch):
  case HASH_FUN(Cache_State_BUSY_INTR, Cache_Event_MakeReadUnique):
  ...
  case HASH_FUN(Cache_State_BUSY_INTR, Cache_Event_WriteUniqueZero):
  case HASH_FUN(Cache_State_BUSY_INTR, Cache_Event_AtomicReturn):
  case HASH_FUN(Cache_State_BUSY_INTR, Cache_Event_AtomicReturn_PoC):
  case HASH_FUN(Cache_State_BUSY_INTR, Cache_Event_AtomicReturn_PoC_Alloc):
  case HASH_FUN(Cache_State_BUSY_INTR, Cache_Event_AtomicNoReturn):
  case HASH_FUN(Cache_State_BUSY_INTR, Cache_Event_AtomicNoReturn_PoC):
  case HASH_FUN(Cache_State_BUSY_INTR, Cache_Event_AtomicNoReturn_PoC_Alloc):
  case HASH_FUN(Cache_State_BUSY_INTR, Cache_Event_StashOnceShared):
  case HASH_FUN(Cache_State_BUSY_INTR, Cache_Event_StashOnceUnique):
  ...
  case HASH_FUN(Cache_State_BUSY_BLKD, Cache_Event_SendAtomicReturn):
    next_state = Cache_State_BUSY_INTR; m_curTransitionNextState = next_state;
    if (!(*m_reqOut_ptr).areNSlotsAvailable(1, clockEdge()))
        return TransitionResult_ResourceStall;
    if (!checkResourceAvailable(Cache_RequestType_DestinationAvailable, addr)) {
    ...
    recordRequestType(Cache_RequestType_DestinationAvailable, addr);
    Pop_TriggerQueue(m_tbe_ptr, m_cache_entry_ptr, addr);
    Send_AtomicReturn(m_tbe_ptr, m_cache_entry_ptr, addr);
    Profile_OutgoingStart(m_tbe_ptr, m_cache_entry_ptr, addr);
    ProcessNextState_ClearPending(m_tbe_ptr, m_cache_entry_ptr, addr);
    return TransitionResult_Valid;

  case HASH_FUN(Cache_State_BUSY_BLKD, Cache_Event_SendAtomicNoReturn):
    next_state = Cache_State_BUSY_INTR; m_curTransitionNextState = next_state;
    if (!(*m_reqOut_ptr).areNSlotsAvailable(1, clockEdge()))
        return TransitionResult_ResourceStall;
    if (!checkResourceAvailable(Cache_RequestType_DestinationAvailable, addr)) {
    ...
    recordRequestType(Cache_RequestType_DestinationAvailable, addr);
    Pop_TriggerQueue(m_tbe_ptr, m_cache_entry_ptr, addr);
    Send_AtomicNoReturn(m_tbe_ptr, m_cache_entry_ptr, addr);
    Profile_OutgoingStart(m_tbe_ptr, m_cache_entry_ptr, addr);
    ProcessNextState_ClearPending(m_tbe_ptr, m_cache_entry_ptr, addr);
    return TransitionResult_Valid;
  ...
  case HASH_FUN(Cache_State_BUSY_BLKD, Cache_Event_SendARData):
  case HASH_FUN(Cache_State_BUSY_INTR, Cache_Event_SendARData):
    Pop_TriggerQueue(m_tbe_ptr, m_cache_entry_ptr, addr);
    Send_ARData(m_tbe_ptr, m_cache_entry_ptr, addr);
    CheckARComp(m_tbe_ptr, m_cache_entry_ptr, addr);
    ProcessNextState_ClearPending(m_tbe_ptr, m_cache_entry_ptr, addr);
    return TransitionResult_Valid;

  case HASH_FUN(Cache_State_BUSY_BLKD, Cache_Event_SendANRData):
  case HASH_FUN(Cache_State_BUSY_INTR, Cache_Event_SendANRData):
    Pop_TriggerQueue(m_tbe_ptr, m_cache_entry_ptr, addr);
    Callback_AtomicNoReturn(m_tbe_ptr, m_cache_entry_ptr, addr);
    Send_ANRData(m_tbe_ptr, m_cache_entry_ptr, addr);
    CheckANRComp(m_tbe_ptr, m_cache_entry_ptr, addr);
    ProcessNextState_ClearPending(m_tbe_ptr, m_cache_entry_ptr, addr);
    return TransitionResult_Valid;
  ...
  case HASH_FUN(Cache_State_BUSY_BLKD, Cache_Event_SendCompUDResp):
    if (!(*m_rspOut_ptr).areNSlotsAvailable(1, clockEdge()))
        return TransitionResult_ResourceStall;
    Pop_TriggerQueue(m_tbe_ptr, m_cache_entry_ptr, addr);
    Send_CompUD_PD(m_tbe_ptr, m_cache_entry_ptr, addr);
    ProcessNextState_ClearPending(m_tbe_ptr, m_cache_entry_ptr, addr);
    return TransitionResult_Valid;
  ...
  case HASH_FUN(Cache_State_BUSY_BLKD, Cache_Event_SendMakeReadUnique):
    next_state = Cache_State_BUSY_INTR; m_curTransitionNextState = next_state;
    if (!(*m_reqOut_ptr).areNSlotsAvailable(1, clockEdge()))
        return TransitionResult_ResourceStall;
    if (!checkResourceAvailable(Cache_RequestType_DestinationAvailable, addr)) {
    ...
    recordRequestType(Cache_RequestType_DestinationAvailable, addr);
    Pop_TriggerQueue(m_tbe_ptr, m_cache_entry_ptr, addr);
    Send_MakeReadUnique(m_tbe_ptr, m_cache_entry_ptr, addr);
    Profile_OutgoingStart(m_tbe_ptr, m_cache_entry_ptr, addr);
    ProcessNextState_ClearPending(m_tbe_ptr, m_cache_entry_ptr, addr);
    return TransitionResult_Valid;
  ...
  case HASH_FUN(Cache_State_BUSY_BLKD, Cache_Event_SendDBIDResp_WUZ):
    if (!(*m_rspOut_ptr).areNSlotsAvailable(1, clockEdge()))
        return TransitionResult_ResourceStall;
    Pop_TriggerQueue(m_tbe_ptr, m_cache_entry_ptr, addr);
  ...
  case HASH_FUN(Cache_State_BUSY_BLKD, Cache_Event_SendCompDBIDResp_ANR):
    if (!(*m_rspOut_ptr).areNSlotsAvailable(1, clockEdge()))
        return TransitionResult_ResourceStall;
    Pop_TriggerQueue(m_tbe_ptr, m_cache_entry_ptr, addr);
    ExpectNCBWrData_A(m_tbe_ptr, m_cache_entry_ptr, addr);
    Send_CompDBIDResp(m_tbe_ptr, m_cache_entry_ptr, addr);
    ProcessNextState_ClearPending(m_tbe_ptr, m_cache_entry_ptr, addr);
    return TransitionResult_Valid;

  case HASH_FUN(Cache_State_BUSY_BLKD, Cache_Event_SendDBIDResp_AR):
    if (!(*m_rspOut_ptr).areNSlotsAvailable(1, clockEdge()))
        return TransitionResult_ResourceStall;
    Pop_TriggerQueue(m_tbe_ptr, m_cache_entry_ptr, addr);
    ExpectNCBWrData_A(m_tbe_ptr, m_cache_entry_ptr, addr);
  ...
  case HASH_FUN(Cache_State_BUSY_BLKD, Cache_Event_SendCompData_AR):
  case HASH_FUN(Cache_State_BUSY_INTR, Cache_Event_SendCompData_AR):
    Pop_TriggerQueue(m_tbe_ptr, m_cache_entry_ptr, addr);
    Send_CompData_AR(m_tbe_ptr, m_cache_entry_ptr, addr);
    ProcessNextState_ClearPending(m_tbe_ptr, m_cache_entry_ptr, addr);
    return TransitionResult_Valid;
  ...
  case HASH_FUN(Cache_State_BUSY_INTR, Cache_Event_NCBWrData):
    ...
    UpdateDataState_FromADataResp(m_tbe_ptr, m_cache_entry_ptr, addr);
    Pop_DataInQueue(m_tbe_ptr, m_cache_entry_ptr, addr);
    ProcessNextState(m_tbe_ptr, m_cache_entry_ptr, addr);
    return TransitionResult_Valid;
  ...
  case HASH_FUN(Cache_State_BUSY_BLKD, Cache_Event_SnpRespData_SD_Fwded_SC):
  ...
  case HASH_FUN(Cache_State_BUSY_INTR, Cache_Event_CompDBIDResp):
    next_state = Cache_State_BUSY_BLKD; m_curTransitionNextState = next_state;
    Receive_ReqResp(m_tbe_ptr, m_cache_entry_ptr, addr);
    Receive_ReqResp_CopyDBID(m_tbe_ptr, m_cache_entry_ptr, addr);
    Profile_OutgoingEnd_DatalessResp(m_tbe_ptr, m_cache_entry_ptr, addr);
    Pop_RespInQueue(m_tbe_ptr, m_cache_entry_ptr, addr);
    ProcessNextState(m_tbe_ptr, m_cache_entry_ptr, addr);
    return TransitionResult_Valid;
  ...
  case HASH_FUN(Cache_State_BUSY_BLKD, Cache_Event_DBIDResp):
    Receive_ReqResp(m_tbe_ptr, m_cache_entry_ptr, addr);
    Receive_ReqResp_CopyDBID(m_tbe_ptr, m_cache_entry_ptr, addr);
    Receive_ReqResp_WUNeedComp(m_tbe_ptr, m_cache_entry_ptr, addr);
    Pop_RespInQueue(m_tbe_ptr, m_cache_entry_ptr, addr);
  ...
  case HASH_FUN(Cache_State_BUSY_INTR, Cache_Event_WriteZero):
    Pop_TriggerQueue(m_tbe_ptr, m_cache_entry_ptr, addr);
    WriteZero(m_tbe_ptr, m_cache_entry_ptr, addr);
    ProcessNextState_ClearPending(m_tbe_ptr, m_cache_entry_ptr, addr);
    return TransitionResult_Valid;
