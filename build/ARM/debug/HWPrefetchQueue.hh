/**
 * DO NOT EDIT THIS FILE!
 * File automatically generated by
 *   build_tools/debugflaghh.py:143
 */

#ifndef __DEBUG_HWPrefetchQueue_HH__
#define __DEBUG_HWPrefetchQueue_HH__

#include "base/compiler.hh" // For namespace deprecation
#include "base/debug.hh"
namespace gem5
{

namespace debug
{

namespace unions
{
inline union HWPrefetchQueue
{
    ~HWPrefetchQueue() {}
    SimpleFlag flagHWPrefetchQueue;

    HWPrefetchQueue() : flagHWPrefetchQueue("HWPrefetchQueue", "", false) {}

} instanceHWPrefetchQueue;
} // namespace unions

inline constexpr const auto& HWPrefetchQueue =
    ::gem5::debug::unions::instanceHWPrefetchQueue.flagHWPrefetchQueue;

} // namespace debug
} // namespace gem5

#endif // __DEBUG_HWPrefetchQueue_HH__
